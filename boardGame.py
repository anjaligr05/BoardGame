...
def possible_raid_pos(localplayer,i,j):
    ...
    arg1=board_beg[i-1][j]
    
    if((i>=1 and board_beg[i-1][j]==localplayer) or(i<4 and board_beg[i+1][j]==localplayer)or(j>=1 and board_beg[i][j-1]==localplayer)or(j<4 and board_beg[i][j+1]==localplayer) ):
        return True
    else:
        return False
...
def min_value(localplayer,cutoff,depth,curpos,prune,a,b):
    ...
    if depth==cutoff:
        if localplayer==opponent:
            ...
            print_traverse_log(curpos,depth,score,a,b)
            #if task in [2,3]:
            #  traverse_output.write("\n"+str(curpos)+","+str(depth)+","+printInf(str(v)))
            #if task==3:
            #  traverse_output.write(","+printInf(str(a))+","+printInf(str(b)))
    ...
    if i==-1 and j==-1:
        if localplayer==opponent:
            score1=player_score-opponent_score
            print_traverse_log(curpos,depth,score1,a,b)
            #if task in [2,3]:
            #  traverse_output.write("\n"+str(curpos)+","+str(depth)+","+printInf(str(v)))
            #if task==3:
            #  traverse_output.write(","+printInf(str(a))+","+printInf(str(b)))
